...
public abstract class RoleResource {
    ...
    protected Set<RoleRepresentation> getParentsRoles(RoleModel role, boolean briefRepresentation) {
        if(briefRepresentation) {
            return role.getParentsStream().map(ModelToRepresentation::toBriefRepresentation).collect(Collectors.toSet());
        }

        return role.getParentsStream().map(ModelToRepresentation::toRepresentation).collect(Collectors.toSet());
    }
}
